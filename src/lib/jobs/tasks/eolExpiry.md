...
**Purpose:** Cron task that hard-deletes wormhole connections whose end-of-life timer has elapsed (`is_eol AND eol_at < now() - EOL_CONNECTION_EXPIRY_SECONDS`) on maps that opt in via `ap_map.delete_eol_connections`. Stage 11.2.
